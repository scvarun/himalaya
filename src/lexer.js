 ...
 export function lex (state) {
   ...
   while (state.cursor < len) {
     const isShortcode = startsWith(str, '[', state.cursor)
     if (isShortcode) {
       lexShortcode(state)
       continue
     }
 ...
 export function lexText (state) {
  ...
  // const textEnd = str.indexOf('<')
   ...
   const shortcodeStart = str.indexOf('[', cursor)
 ...
 */
 ...
 export function lexShortcode (state) {
   const {str} = state
   {
     const secondChar = str.charAt(state.cursor + 1)
     const close = secondChar === '/'
     state.tokens.push({type: 'shortcode-start', close})
     state.cursor += close ? 2 : 1
   }
   const tagName = lexShortcodeName(state)
   lexShortcodeAttributes(state)
   {
     const firstChar = str.charAt(state.cursor)
     const close = firstChar === '/'
     state.tokens.push({type: 'shortcode-end', close})
     state.cursor += close ? 2 : 1
   }
   return tagName
 }

 export function lexShortcodeName (state) {
   const {str, cursor} = state
   const len = str.length
   let start = cursor
   while (start < len) {
 ...
 export function lexShortcodeAttributes (state) {
   const {str, tokens} = state
   let cursor = state.cursor
   let quote = null // null, single-, or double-quote
   let wordBegin = cursor // index of word start
