...
exports.lexShortcode = lexShortcode;
exports.lexShortcodeName = lexShortcodeName;
exports.lexShortcodeAttributes = lexShortcodeAttributes;
...
function lex(state) {
  ...
  while (state.cursor < len) {
    var isShortcode = (0, _compat.startsWith)(str, '[', state.cursor);
    if (isShortcode) {
      lexShortcode(state);
      continue;
    }
...
function lexText(state) {
  var str = state.str,
      cursor = state.cursor;
  // const textEnd = str.indexOf('<')

  var type = 'text';

  var textEnd = -1;

  var tagStart = str.indexOf('<', cursor);
  var shortcodeStart = str.indexOf('[', cursor);

  if (tagStart !== -1 && (shortcodeStart === -1 || tagStart < shortcodeStart)) {
    textEnd = tagStart;
  } else if (shortcodeStart !== -1 && (tagStart === -1 || tagStart > shortcodeStart)) {
    textEnd = shortcodeStart;
  }
...
/*
 * Code for lexing shortcodes
 * -------------------------
 *  Similar to lexing tags
 *
 */

function lexShortcode(state) {
  var str = state.str;

  {
    var secondChar = str.charAt(state.cursor + 1);
    var close = secondChar === '/';
    state.tokens.push({ type: 'shortcode-start', close: close });
    state.cursor += close ? 2 : 1;
  }
  var tagName = lexShortcodeName(state);
  lexShortcodeAttributes(state);
  {
    var firstChar = str.charAt(state.cursor);
    var _close2 = firstChar === '/';
    state.tokens.push({ type: 'shortcode-end', close: _close2 });
    state.cursor += _close2 ? 2 : 1;
  }
  return tagName;
}

function lexShortcodeName(state) {
  var str = state.str,
      cursor = state.cursor;
  ...
  var end = start + 1;
  while (end < len) {
    var _char2 = str.charAt(end);
    var _isTagChar2 = !(isWhitespaceChar(_char2) || _char2 === '/' || _char2 === ']');
    if (!_isTagChar2) break;
    end++;
  }

  state.cursor = end;
  var tagName = str.slice(start, end);
  state.tokens.push({ type: 'shortcode', content: tagName });
  return tagName;
}

function lexShortcodeAttributes(state) {
  var str = state.str,
      tokens = state.tokens;
  ...
  for (var i = 0; i < wLen; i++) {
    ...
    if (isNotPair) {
      ...
      if (secondWord && (0, _compat.startsWith)(secondWord, '=')) {
        ...
        i += 1;
        if (thirdWord) {
          var _newWord4 = word + '=' + thirdWord;
          tokens.push({ type: type, content: _newWord4 });
          i += 1;
          continue;
        }
      }
    ...
    if ((0, _compat.endsWith)(word, '=')) {
      var _secondWord2 = words[i + 1];
      if (_secondWord2 && !(0, _compat.stringIncludes)(_secondWord2, '=')) {
        var _newWord6 = word + _secondWord2;
        tokens.push({ type: type, content: _newWord6 });
        i += 1;
        continue;
      }

      var _newWord5 = word.slice(0, -1);
      tokens.push({ type: type, content: _newWord5 });
      continue;
    }
